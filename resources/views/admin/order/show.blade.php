...
@php
    ...
    $coupen = json_decode($order->coupen);
@endphp
        ...
        <div class="invoice">
            ...
            <div class="row">
                ...
                <div class="row">
                    <div class="col-md-6">
                    <address>
                        <strong>Payment Informtion:</strong><br>
                    ...
                    <address>
                        ...
                        <b>{{ date('Y-F-d', $order->createdAt) }}</b><br><br>
                    </address>
                    </div>
                </div>
                </div>
            ...
            <div class="row mt-4">
                ...
                <div class="table-responsive">
                    ...
                    @php
                        $variants = json_decode($products->variants)
                    @endphp

                        <tr>
                            <td>{{ ++$loop->index }}</td>
                            @if ($products->product->slug)
                                <td><a href="{{ route('product-detail', $products->product->slug) }}" target="_blank">{{ $products->product_name }}</a></td>
                            @else
                            ...
                            <td class="text-center">{{ $products->vendor->shop_name }}</td>
                            <td class="text-center">{{ $settings->currency_icon }} {{ $products->unit_price }}</td>
                            <td class="text-center">{{ $products->qty }}</td>
                            <td class="text-right">{{ $settings->currency_icon }} {{ ($products->unit_price * $products->qty) + $products->variants_total}}</td>
                        </tr>
                    @endforeach
                ...
                <div class="row mt-4">
                    <div class="col-lg-8">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="order-status">Payment Status</label>
                    ...
                    <div class="invoice-detail-item">
                        <div class="invoice-detail-name">Coupen(-)</div>
                        <div class="invoice-detail-value">{{ $settings->currency_icon}} {{ $coupen->discount }}</div>
                    </div>
                    <hr class="mt-2 mb-2">
                    <div class="invoice-detail-item">
